...
class OpenAIClient:
    def __init__(
        ...
        *,
        ...
        self._host = "https://node2.chatshare.biz/"
    ...
    async def post_init(self):
        ...
        # await self.playwright_page.goto("https://node2.chatshare.biz/chatgpt/login")
        await self.playwright_page.goto("https://node2.chatshare.biz/")
        await asyncio.sleep(2)
        login_button = await self.playwright_page.query_selector("text=log in")  # 或者使用具体的选择器，如：button#login
        if login_button:
            print("Log in button found. Clicking it.")
            await login_button.click()
            await self.playwright_page.fill("input[name='username']", "1T2Z4OBXL7")
            await self.playwright_page.fill("input[name='password']", "295636")
                # # 提交登录表单
            await self.playwright_page.click("button[type='submit']")
            await self.navigate_with_retry(self._host)
        else:
    ...
    async def navigate_with_retry(self, url, retries=3, delay=10):
        for i in range(retries):
            try:
                await self.playwright_page.goto(url)
                logger.error(f"错误")
            except Exception as e:
                logger.error(f"Failed to navigate to {url}: {e}. Retry {i + 1}/{retries}")
                # 如果导航成功，则尝试点击页面中的"TT-10"文字
                try:
                    t10_button = self.playwright_page.locator('text="TT-10"')
                    await t10_button.click()
                    logger.info(f"Successfully clicked TT-10 text on attempt {i + 1}")
                except Exception as click_exception:
                    logger.error(f"Failed to click TT-10 text: {click_exception}")
                return
                if i < retries - 1:
    ...
    async def __handle_response(self, response: Response):
        try:
            ...
            logger.error(f"Error in __handle_response: {e}")
        ...
        if model_map.get(model_name) != self.current_model:
            ...
            try:
                await self.playwright_page.wait_for_selector(
                    'div[role="menu"]', state="attached", timeout=100
                )
            except TimeoutError:
                pass
    ...
    async def new_conversation(self):
        ...
        if not url.startswith("https://node2.chatshare.biz/"):
            # TODO
            # 如果这里跳到auth页面，需要处理
    ...
    async def close(self):
        ...
        await self.playwright_page.context.close()
...
async def main():
    async with async_playwright() as p:
        ...
        client = OpenAIClient(playwright_page=page)
